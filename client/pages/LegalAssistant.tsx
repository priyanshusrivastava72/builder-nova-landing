...
export default function LegalAssistant() {
  ...
  useEffect(() => {
    idxRef.current = buildIndex(CASES);
  }, []);
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                We\'ll retrieve relevant cases and compose an answer
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {m.sources && m.sources.length > 0 && (
                      <div className="mt-2 space-y-2">
                        {m.sources.map((s, j) => (
                          <div
                            key={j}
                            className="text-sm p-3 bg-white border border-legal-200 rounded-lg"
                          >
                            <div className="font-medium text-legal-900 flex items-center gap-2">
                              <BookOpen className="w-4 h-4" /> {s.title}
                            </div>
              ...
              <div className="mt-4 flex gap-2">
                <Input
                  ...
                  placeholder="e.g. What is an audit clause and how is it enforced?"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") ask();
                  }}
                />
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    key={s}
                    onClick={() => setInput(s)}
                    className="px-3 py-1 rounded-full bg-legal-50 border border-legal-200 text-legal-700 hover:bg-legal-100 text-sm"
                  >
                    {s}
                  </button>
                ))}
...
function pickSnippet(text: string, q: string) {
  ...
  for (const s of sentences) {
    const sc =
      terms.reduce((acc, t) => acc + (s.toLowerCase().includes(t) ? 1 : 0), 0) +
      s.length / 500;
    if (sc > bestScore) {
      bestScore = sc;
      best = s;
